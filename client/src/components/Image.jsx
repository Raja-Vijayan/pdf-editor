...
export default function Image({ myImages, setAddElement, isLoaded }) {
    ...
    return (
        <div className='w-full'>
            <p className='sticky top-0 border-[1px] border-solid py-[7px] px-1 w-full shadow-sm bg-[#992bff] text-white' style={{ clipPath: "polygon(0 1%, 100% 0%, 91% 100%, 0% 100%)" }}>Select Your Template Images</p>
            <div className='w-full'>
                    ...
                    {myImages.map((img, i) => {
                        return (
                            <div className='p-2 bg-[rgba(255,255,255,0.6)] cursor-pointer mt-1' onClick={() => setAddElement('image', `data:image/png;base64,${img.file_base64}`)}
                                ...
                                <img src={`data:image/png;base64,${img.file_base64}`} alt={img.name} className='w-[100px] h-[100px] rounded-md border-[2px] border-solid border-[rgba(0,0,0,0.1)]' loading='lazy' draggable
                                    onDragStart={() => { setHoveredItem(false); handleDragStart('image', `data:image/png;base64,${img.file_base64}`) }}
